...
export async function sendInvoiceEmail(invoiceId:string){
 ...
 if(!payment){const created=await sql`INSERT INTO moms_ops.payments(booking_id,invoice_id,amount_cents,currency,status) SELECT appointment_id,id,amount_due_cents,'usd','pending' FROM moms_ops.invoices WHERE id=${invoiceId}::uuid RETURNING id`;payment=created[0];}
