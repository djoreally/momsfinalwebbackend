...
export async function sendInvoiceEmail(invoiceId:string){
 ...
 const response=await fetch(RESEND_API,{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({from,to:[invoice.email],bcc:notifications.ownerBccEmail?[notifications.ownerBccEmail]:undefined,subject:`MOMS invoice — ${money(Number(invoice.amountDueCents))} due`,html})});
